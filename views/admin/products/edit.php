    ...
    <div class="admin-header">
        <div class="container">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="mb-0">Admin - Chỉnh sửa sản phẩm</h1>
                </div>
                <div>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-9">
                <div class="form-container">
                    <form method="POST" action="<?= BASE_URL ?>?act=product-update">
                        <input type="hidden" name="product_id" value="<?= (int) $product['id'] ?>">

                        <div class="row g-3">
                            <div class="col-md-8">
                                <label for="name" class="form-label"><strong>Tên sản phẩm <span class="text-danger">*</span></strong></label>
                                <input type="text" class="form-control" id="name" name="name" required value="<?= e($product['name']) ?>">
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" id="brand_id" name="brand_id" required>
                                    <?php foreach ($brands as $brand) : ?>
                                        <option value="<?= (int) $brand['id'] ?>" <?= (int) $product['brand_id'] === (int) $brand['id'] ? 'selected' : '' ?>><?= e($brand['name']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" min="0" step="1000" class="form-control" id="price" name="price" required value="<?= (float) $product['price'] ?>">
                            </div>
                            <div class="col-md-4">
                                <label for="sale_price" class="form-label"><strong>Giá khuyến mãi</strong></label>
                                <input type="number" min="0" step="1000" class="form-control" id="sale_price" name="sale_price" value="<?= $product['sale_price'] !== null ? (float) $product['sale_price'] : '' ?>">
                            </div>
                            <div class="col-md-4">
                                <label for="stock" class="form-label"><strong>Tồn kho</strong></label>
                                <input type="number" min="0" class="form-control" id="stock" name="stock" value="<?= (int) $product['stock'] ?>">
                            </div>

                            <div class="col-12">
                                <label for="thumbnail" class="form-label"><strong>Ảnh đại diện (URL)</strong></label>
                                <input type="text" class="form-control" id="thumbnail" name="thumbnail" value="<?= e($product['thumbnail'] ?? '') ?>">
                            </div>
                            <div class="col-12">
                            ...
                            <div class="col-12 d-flex flex-wrap gap-4">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" <?= (int) $product['is_active'] === 1 ? 'checked' : '' ?>>
                                    <label class="form-check-label" for="is_active">Đang hiển thị</label>
                                </div>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="is_featured" name="is_featured" value="1" <?= (int) $product['is_featured'] === 1 ? 'checked' : '' ?>>
